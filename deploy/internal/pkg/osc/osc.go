...
// ClearOscCookieCache removes the osc cookie jar to force fresh authentication.
// osc caches auth cookies that can become stale, causing auth failures in
// long-running services.
func ClearOscCookieCache() {
	home, err := os.UserHomeDir()
	if err != nil {
		return
	}
	cookiePath := filepath.Join(home, ".local", "state", "osc", "cookiejar")
	if _, err := os.Stat(cookiePath); err == nil {
		if err := os.Remove(cookiePath); err != nil {
			slog.Warn("Failed to clear osc cookie cache", "path", cookiePath, "error", err)
		} else {
			slog.Debug("Cleared osc cookie cache", "path", cookiePath)
		}
	}
}
